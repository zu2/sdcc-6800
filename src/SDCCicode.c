...
geniCodeMultiply (operand * left, operand * right, RESULT_TYPE resultType)
{
  ...
  if ((p2 > 0) && !IS_FLOAT (letype) && !IS_FIXED (letype) &&
      !((resultType == RESULT_TYPE_INT) && (getSize (resType) != getSize (ltype)) && !(TARGET_Z80_LIKE || TARGET_MOS6502_LIKE || TARGET_MC6800_LIKE || TARGET_F8_LIKE || TARGET_IS_STM8 && p2 == 1) /* Mimic old behaviour that tested port->muldiv, which was zero for stm8 and z80-like only. Someone should look into what really makes sense here. */) &&
