  ...
  if ((level > 0) && !SPEC_STAT (dest))
    {
      ...
      else if (TARGET_IS_DS390 || TARGET_IS_DS400 || options.useXstack || TARGET_IS_HC08 || TARGET_IS_S08 || TARGET_IS_MC6800)
        {
          if (destScls == S_FIXED)
            destScls = S_XDATA;
          if (srcScls == S_FIXED)
      ...
      for (muldivmod = 0; muldivmod < 3; muldivmod++)
        {
          ...
          muldiv[muldivmod][bwd][su] = funcOfType2 (_mangleFunctionName (dbuf_c_str (&dbuf)),
            multypes[((TARGET_IS_PIC16 || TARGET_IS_PIC14 || TARGET_IS_STM8 || TARGET_Z80_LIKE || TARGET_PDK_LIKE || TARGET_MOS6502_LIKE || TARGET_MC6800_LIKE || TARGET_F8_LIKE) && bwd == 0) ? 1 : bwd][(bool)su],
              multypes[bwd][su % 2],
